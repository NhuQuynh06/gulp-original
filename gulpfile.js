...
async function copy_js() {
  return src(paths.js_source).pipe(dest(paths.js_dist));
}

// > copy and prettyfy HTML
async function copy_html() {
  return (
    src([paths.html_source, ignoreFiles.join()])
      .pipe(
        fileinclude({
          prefix: "@@",
          basepath: "@file",
...
async function copy_font_dist() {
  return src(paths.fonts_source).pipe(dest(paths.fonts_dist));
}
...
function copy_img_dist() {
  return src(paths.img_source).pipe(dest(paths.img_dist));
}
...
async function watch_dev() {
  ...
  watch("./source/desktop/component/**/*.scss", series(compile_scss_dist, refresh_on_change));
  watch("./source/mobile/component/**/*.scss", series(compile_scss_dist, refresh_on_change));
