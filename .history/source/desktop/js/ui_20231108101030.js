...
const background = function () {
  let porter = $(".swiper-slide-active .content video").attr("poster");
  $(".swiper .bg-blur").css(
    "background",
    "url(" + porter + ") center center/ 100% auto no-repeat"
  );
};
// short video
if ($(".video-detail").length) {
  ...
  $(".video-detail .mySwiper").on("wheel", function (e) {
    e.preventDefault();
    background();
  });
  // toggle icon
  var video = $(".option .ic-video");
  var volumne = $(".option .ic-volume");
  video.click(function (e) {
    e.preventDefault();
    $(this).toggleClass("ic-pause");
  });
  volumne.click(function (e) {
    e.preventDefault();
    $(this).toggleClass("ic-volume-slash");
  });
}
$(".short-video .story").click(function (e) {
  e.preventDefault();
  setTimeout(() => {
...
if ($(".longform").length) {
  var swiper = new Swiper(".photo-detail-thumb", {
    loop: true,
    slidesPerView: 8,
    spaceBetween: 20,
    freeMode: true,
  ...
  var swiper2 = new Swiper(".photo-detail-img", {
    autoHeight: true,
    loop: true,
    spaceBetween: 20,
    navigation: {
